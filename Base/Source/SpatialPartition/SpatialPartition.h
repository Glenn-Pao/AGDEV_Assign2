...
public:
	...
	// Calculate the squared distance from camera to a grid's centrepoint
	float CalculateDistanceSquare(Vector3 thePosition, Vector3 theDirection, const int xIndex, const int yIndex);

	//Check a position for collision with objects in any of the grid
	bool CheckForCollision(Vector3 position);
	//check 2 position for collision with objects in any of the grid
	bool CheckForCollision(Vector3 position_start, Vector3 position_end);
	//check collision between 2 sceneNodes
	bool CheckForCollision(CSceneNode* n);
